...
public class TypeCheckVisitor implements ASTVisitor, TypeConstants {
	...
	public Object visitExpressionLValue(ExpressionLValue expressionLValue, Object arg) throws Exception 
	{
		...
		if(dec==null)
			throw new TypeCheckException("Variable "+ident+" no declared previously", expressionLValue);
		
		if(dec instanceof VarDec)
		{
			VarDec varDec = (VarDec) dec;
			if(varDec.type.getJVMType().equalsIgnoreCase("Ljava/util/List<I>;"))
				expressionLValue.setType(intType);
			else if(varDec.type.getJVMType().equalsIgnoreCase("Ljava/util/List<Z>;"))
				expressionLValue.setType(booleanType);
			else if(varDec.type.getJVMType().equalsIgnoreCase("Ljava/util/List<Ljava/lang/String;>;"))
				expressionLValue.setType(stringType);
			else
			expressionLValue.setType(varDec.type.getJVMType());
			
			if(!expressionLValue.expression.getType().equals(intType))
				throw new TypeCheckException("Argument in ExpressionLValue should be of type integer", expressionLValue);
		}
	...
	{
		//System.out.println(listOrMapElemExpression.expression);
		...
		if(dec==null)
			throw new TypeCheckException("Variable "+ident+" no declared previously", listOrMapElemExpression);
		...
		if(dec instanceof VarDec)
		{
			...
			listOrMapElemExpression.setType(varDec.type.getJVMType());
		if(!listOrMapElemExpression.expression.getType().equals(intType))
			throw new TypeCheckException("Illegal argument in ListOrMapElemExpression", listOrMapElemExpression);
		}
	 ...
	 */
	@Override
	public Object visitRangeExpression(RangeExpression rangeExpression, Object arg) throws Exception 
	{
		String type1=(String)rangeExpression.lower.visit(this, arg);
		String type2=(String)rangeExpression.upper.visit(this, arg);
		
		if(!(type1.equals(intType)) || !(type2.equals(intType)) )
			throw new TypeCheckException("Range Expression should contain int Type", rangeExpression);
	...
	//	{
			if(!symbolTable.insert(ident, varDec))
